import pygame, random, sys, inventorydata # type: ignore [this is so vscode doesn't yell at me]
...
itemNameText = pygame.font.Font(pygame.font.match_font('courier'), 22)
quantityText = pygame.font.Font(pygame.font.match_font('courier'), 18)
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            # rect for collisionpoint, itemName for display, (row/column inven slot location), rawData
            # renderedInventorySlots[str(xPos) + str(yPos)] = (buttonRect, itemName, (rowInt, columnInt), rawData)
            for xPosYPosString, slotItemData in renderedInventorySlots.items():
